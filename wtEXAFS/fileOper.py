...
# --------- 构建用户定义kW数据集（多列数据处理） ---------
def createkWforWT():
    # --------- 对当小波开始从末端跑出时，k序列的边缘附近会出现问题。为了最大限度地减少这些问题，使用零填充k序列的首尾。 ---------
...
def inverseWaveleTransformation():
    ...
    # --------- 获取逆小波变换所需要的小波常数 ---------
    wavelet_coef = para.Parameters.wavelet_coef
    # --------- 构造序列 ---------
    k_input = valueList("k+")
    # ------- 准备获取小波逆变换的结果 -----------
    chi_recon = []
    for i in range(2, col_limit):
        ...
        temp = waveletMethod.iWT(w_matrix, energy_coef, wavelet_coef, para.Parameters.dR)
